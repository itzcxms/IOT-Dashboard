...
function DropDownTempSatisfaction({ periode, listePeriodes, editPeriode }) {
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-fit" align="end">
        <DropdownMenuGroup>
          {listePeriodes.map((value) => {
            if (value !== periode) {
              return (
                <DropdownMenuItem
                  onClick={async () => editPeriode(value)}
                  key={value}
                >
                  {value}
                </DropdownMenuItem>
...
export default DropDownTempSatisfaction;
